...
numberOfMonths: numOfMonths
	...
	//請求日設定コントロール
	$("#shime_year").change(function() {
		set_shime_date();
	});
	$("#shime_month").change(function() {
		set_shime_date();
	});
	...
	$(document).on("change", "#shime_group, #shime_month, #shime_year", function() {
		get_moto();
	});
	...
	//元請け選択コントロール
	$(document).on("change", "#sel_moto_nobranch", function() {
		$("#sel_moto_id").val("0");
	});
	$(document).on("change", "#sel_moto_id", function() {
		$("#sel_moto_nobranch").val("0");
	});
...
function set_shime_date() {
	var year = to_num($("#shime_year").val());
	var month = to_num($("#shime_month").val());
	var shime_group = to_num($("#shime_group").val());
	
	if(year != 0 && year > 2010 && month >= 1 && month <= 12 && shime_group != 0) {
		//締めグループで分類
		
		if(shime_group < 30) {
			//5 10 日
			var tmp_date = year + "-" + month + "-" + shime_group;
			$("#shime_date").val(tmp_date);	
		}
		else if(shime_group == 50) {
			//スポット請求は当日
			$("#shime_date").val(get_today());	
		}
		else {
			...
			var tmp_date = year + "-" + month + "-" + lastday;
			//月末日計算
			$("#shime_date").val(tmp_date);	
		}
	}
...
//----------------------個別領域 ここから----------------------------
	...
	var sel_moto_id = $("#sel_moto_id").val();
	var sel_moto_nobranch = $("#sel_moto_nobranch").val();
	...
	//元請け両方チェック
	if($("#sel_moto_id").length) {
		if(sel_moto_id != "0" && sel_moto_nobranch != "0") {
			show_fail("元請けはどちらか一方の条件を指定して下さい。");
			return;
		}
	}
	$(".search-area").show();
...
//----------------------個別領域 ここまで----------------------------
	...
	$.ajax({
		...
		cache:		false,
		url:		"ajax.php",
		data:		{parm:[flag, page, orderc,shime_group, shime_date, shime_year, shime_month, sel_moto_id, sel_moto_nobranch]},
		type:		"post",
...
function shime_exec() {
	...
	$.ajax({
		...
		cache:		false,
		url:		"ajax.php",
		data:		{parm:[flag, cbno, shime_date, shime_year, shime_month,shime_group]},
		type:		"post",
		headers: 	{"pragma": "no-cache"},
		success:	function(data, textStatus) {
				$(".main-area").html(data);
...
function 	show_shime_group_list() {
	var flag = "SHOW_SHIME_GROUP";
	...
	$.ajax({
		...
		headers: 	{"pragma": "no-cache"},
		success:	function(data, textStatus) {
				$("#shime_group_span").html(data);
		},
		complete:	function(data, textStatus) {
			//請求日設定コントロール
			$("#shime_group").change(function() {
				set_shime_date();
			});
		},
		error:		function(data, textStatus) {
...
//----------------------個別領域 ここまで----------------------------
	...
	$.ajax({
		...
		cache:		false,
		url:		"get_moto.php",
		data:		{parm:[flag, page, orderc,shime_group, shime_date, shime_year, shime_month]},
		type:		"post",
		headers: 	{"pragma": "no-cache"},
		success:	function(data, textStatus) {
				$("#shime_moto_span").html(data);
		},
		complete:	function(data, textStatus) {
