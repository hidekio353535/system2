...
var editMode = 1;
//----------------------個別領域 ここから----------------------------
var MAIN_TABLE = "matsushima_tantou";
var MAIN_ID_FIELD = "t_id";
//----------------------個別領域 ここまで----------------------------
...
//----------------------個別領域 ここから----------------------------
			$("."+MAIN_ID_FIELD).each(function(i, elm) {
				...
				var t_gencho = $(".t_gencho").eq(i).attr("checked");
				if(t_gencho == "checked")
					t_gencho = 1;
				else
					t_gencho = 0;
				
				var is_show_tantou = $(".is_show_tantou").eq(i).attr("checked");
				if(is_show_tantou == "checked")
					is_show_tantou = 1;
				else
					is_show_tantou = 0;
				...
				sql += "t_tantou = '"+ h($(".t_tantou").eq(i).val()) +"'";
				sql += ",";
				sql += "t_tantou_nik = '"+ h($(".t_tantou_nik").eq(i).val()) +"'";
				sql += ",";
				sql += "t_tel = '"+ h($(".t_tel").eq(i).val()) +"'";
				sql += ",";
				sql += "t_biko = '"+ h($(".t_biko").eq(i).val()) +"'";
				sql += ",";
				sql += "t_gencho = '"+ t_gencho +"'";
				sql += ",";
				sql += "is_show_tantou = '"+ is_show_tantou +"'";
				sql += ",";
				sql += "t_order = '"+ h($(".t_order").eq(i).val()) +"'";
...
//----------------------個別領域 ここから----------------------------
			...
			var t_gencho = $(".t_gencho").eq(0).attr("checked");
			if(t_gencho == "checked")
				t_gencho = 1;
			else
				t_gencho = 0;

			var is_show_tantou = $(".is_show_tantou").eq(0).attr("checked");
			if(is_show_tantou == "checked")
				is_show_tantou = 1;
			else
				is_show_tantou = 0;
			...
			if(id) { //更新
				sql += "UPDATE "+MAIN_TABLE+" SET ";
				sql += "t_tantou = '"+ h($(".t_tantou").eq(0).val()) +"'";
				sql += ",";
				sql += "t_tantou_nik = '"+ h($(".t_tantou_nik").eq(0).val()) +"'";
				sql += ",";
				sql += "t_tel = '"+ h($(".t_tel").eq(0).val()) +"'";
				sql += ",";
				sql += "t_biko = '"+ h($(".t_biko").eq(0).val()) +"'";
				sql += ",";
				sql += "t_gencho = '"+ t_gencho +"'";
				sql += ",";
				sql += "is_show_tantou = '"+ is_show_tantou +"'";
				sql += ",";
				sql += "t_order = '"+ h($(".t_order").eq(0).val()) +"'";
				sql += " WHERE "+MAIN_ID_FIELD+" ="+id+";;";
				
			}
			else {
				//新規
				sql += "INSERT INTO "+MAIN_TABLE+" (t_tantou, t_tantou_nik, t_tel, t_biko, t_gencho, is_show_tantou,t_order) VALUES ( ";
				sql += "'"+h($(".t_tantou").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".t_tantou_nik").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".t_tel").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".t_biko").eq(0).val())+"'";
				sql += ",";
				sql += "'"+t_gencho+"'";
				sql += ",";
				sql += "'"+is_show_tantou+"'";
				sql += ",";
				sql += "'"+h($(".t_order").eq(0).val())+"'";
				sql += ");;";
			}
//----------------------個別領域 ここまで----------------------------
...
//----------------------個別領域 ここから----------------------------
	...
	if($(".t_tantou").eq(0).val() == "") {
		err_flag = true;
		$(".t_tantou_err").eq(0).html("必須項目です");
	}
