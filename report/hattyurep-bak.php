...
$title = "{$COMPANY_NAME} 発注先毎発注レポート {$year}年{$month}月度 (確定版)";
...
while ($row = mysql_fetch_object($query)) {
	...
	s_genba_id = '{$g_id}'
	AND
	s_shime_date >= '{$st_date}'
	AND
	s_shime_date <= '{$end_date}'
	";
...
$tbl .= <<<EOD
	<tr>
		...
		<td colspan="1" class="waku tar"></td>
		<td colspan="1" class="waku tac">現場数：{$num}</td>
