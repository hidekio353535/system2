...
$title = "{$COMPANY_NAME} 発注レポート {$year}年{$month}月度";

$sql = "
SELECT *
FROM matsushima_hat as hat
LEFT OUTER JOIN matsushima_seko as m ON m.seko_id = hat.h_seko_id

WHERE
h_year = '{$year}'
AND
h_month = '$month'
ORDER BY h_id, h_seko_id
";	
...
while ($row = mysql_fetch_object($query)) {
	...
	$h_receipt_yotei_date = $row->h_receipt_yotei_date;
	$hat_kubun = $row->hat_kubun;
...
$tbl .= <<<EOD
	<tr>
		<td colspan="2" class="waku tac">一覧発注伝票数：{$num}</td>
