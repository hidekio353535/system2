...
var editMode = 1;
//----------------------個別領域 ここから----------------------------
var MAIN_TABLE = "matsushima_slip_jv";
...
$(document).ready(function(){
	...
	$("#all-print-s").click(function() {
		...
		if(cbno == "") {
			...
			subwin1=window.open("../print/hat_jv.php?sid="+cbno ,"sub1", "width=950,height=700, scrollbars=yes, resizable=yes");
			subwin1.focus();
		}
	});
...
//----------------------個別領域 ここから----------------------------
	...
	if(date_sel == "work_date") {
		if((!search_st_date && search_end_date) || (search_st_date && !search_end_date)) {
			alert("作業日を指定した場合は開始期間と終了期間の両方を必ず指定して下さい。");
			return;
		}
	}
	...
	//発注先
	if(search_sel_seko != 0) {
		jyoken += "<b>元請：</b>";
		jyoken += $("#search_sel_seko option:selected").text();
		jyoken += "&nbsp;&nbsp;&nbsp;&nbsp;";
	}
...
//----------------------個別領域 ここまで----------------------------
	...
	$.ajax({
		...
		cache:		false,
		url:		"ajax.php",
		data:		{parm:[flag, page, orderc,search_id,search_kw1,search_kw2,shime_year,shime_month,search_sel_seko,search_st_date,search_end_date,date_sel,search_sel_tantou,search_sel_moto,search_sel_kubun,per_page,search_sel_branch, search_g_id]},
		type:		"post",
		headers: 	{"pragma": "no-cache"},
		success:	function(data, textStatus) {
				$(".main-area").html(data);
		...
		complete:	function(data, textStatus) {
			...
			$(".ui-icon-print").click(function(e) {
				...
				subwin2=window.open("../print/hat_jv.php?sid="+id ,"sub2", "width=950,height=700, scrollbars=yes, resizable=yes");
				subwin2.focus();
			});
