...
//----------------------個別領域 ここから----------------------------
	...
	if($search_sel_seko != "" && $search_sel_seko != 0)
		$JYOKEN .= " AND EXISTS(
						SELECT * FROM matsushima_jv_rel as B WHERE jv_seko_id = '{$search_sel_seko}' AND A.s_id = B.jv_slip_id
						) ";
...
//----------------------個別領域 ここまで----------------------------
	...
	//ページ管理用 最終ページ計算
	try {
		$sql = "SELECT * FROM {$VIEW_TABLE} as A
		INNER JOIN matsushima_genba ON matsushima_genba.g_id = A.s_genba_id
		LEFT OUTER JOIN matsushima_kouji_syu_hat ON matsushima_kouji_syu_hat.sy_id = A.s_seko_kubun_id
...
LEFT(kana,1) as kana3
	...
	if($slipno == 0)
		...
		$sql = "UPDATE matsushima_slip_jv SET s_is_print = 1 WHERE s_id in ({$id})";
		$query = mysql_query($sql);
	}
	else
		return;
	...
	if($slipno == 0)
		...
		$sql = "UPDATE matsushima_slip_jv SET s_is_print = 0 WHERE s_id in ({$id})";
		$query = mysql_query($sql);
	}
	else
		return;
