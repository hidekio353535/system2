...
//----------------------個別領域 ここから----------------------------
$VIEW_TABLE = "matsushima_slip_est";
$VIEW_ID_FIELD = "s_id";

$MAIN_TABLE = "matsushima_slip_est";
$MAIN_ID_FIELD = "s_id";
...
//----------------------個別領域 ここまで----------------------------
	...
	try {
		...
		LEFT OUTER JOIN matsushima_est_syu ON matsushima_est_syu.sy_id = {$VIEW_TABLE}.s_seko_kubun_id
...
//----------------------個別領域 ここから----------------------------
	...
	showTh("s_date", "見積日");
	showTh("s_moto_invoice", "見積額");
	showTh("s_mail_send", "メール送信");
	showTh("s_biko", "備考");
...
//----------------------個別領域 ここから----------------------------
			echo "<td class='nw' style='vertical-align:middle'>";
				echo '<input type="checkbox" class="chk" value="'.$row->$VIEW_ID_FIELD.'">
				 ...
				 <img src="../img/b_mit.png" title="見積書表示" class="opa smlb ui-icon-print">
				';
			...
			echo "<td class='nw tac'>";
				echo "<span class=''>";
					...
					if($row->$clm)
						echo "<span style='color:green'>送信済</span>";
					else
						echo "<span style='color:red'>未送信</span>";
				echo "</span>";
			echo "</td>";
...
//----------------------個別領域 ここまで----------------------------
	...
	//表示するレコードが0の場合
		...
		echo "<tr>";
			echo "<td colspan='15' style='text-align:center;color:red'>該当するデータがありません</td>";
		echo "</tr>";
	}
	echo "</table>";
	echo "<input type='hidden' value='￥".number_format($ttl_est)."' id='ttl_est' />";
	...
	//区分
	$sql = "SELECT * FROM matsushima_est_syu WHERE 1";
	$query = @mysql_query($sql);
	$num = @mysql_num_rows($query);
	if(!$query) {
		echo "fail";
...
LEFT(kana,1) as kana3
	...
	if($slipno == 0)
		...
		if($row->s_is_print)
			$sql = "UPDATE matsushima_slip_est SET s_is_print = 0 WHERE s_id = '{$id}'";
		else
			$sql = "UPDATE matsushima_slip_est SET s_is_print = 1 WHERE s_id = '{$id}'";
	...
	if($slipno == 0)
		...
		$sql = "UPDATE matsushima_slip_est SET s_is_print = 1 WHERE s_id in ({$id})";
		$query = mysql_query($sql);
	}
	else if($slipno == 2) {
	...
	if($slipno == 0)
		...
		$sql = "UPDATE matsushima_slip_est SET s_is_print = 0 WHERE s_id in ({$id})";
		$query = mysql_query($sql);
	}
	else if($slipno == 2) {
