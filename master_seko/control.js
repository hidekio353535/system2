...
var editMode = 1;
//----------------------個別領域 ここから----------------------------
var MAIN_TABLE = "matsushima_seko";
var MAIN_ID_FIELD = "seko_id";
//----------------------個別領域 ここまで----------------------------
...
//----------------------個別領域 ここから----------------------------
			...
			$("."+MAIN_ID_FIELD).each(function(i, elm) {
				...
				var is_bankshow = $(".is_bankshow").eq(i).attr("checked");
				if(is_bankshow == "checked")
					is_bankshow = 1;
				else
					is_bankshow = 0;

				var is_schshow = $(".is_schshow").eq(i).attr("checked");
				if(is_schshow == "checked")
					is_schshow = 1;
				else
					is_schshow = 0;

				var is_seko_show = $(".is_seko_show").eq(i).attr("checked");
				if(is_seko_show == "checked")
					is_seko_show = 1;
				else
					is_seko_show = 0;
				...
				sql += ",";
				sql += "atena = '"+ h($(".atena").eq(i).val()) +"'";
				sql += ",";
				sql += "postal = '"+ h($(".postal").eq(i).val()) +"'";
				sql += ",";
				sql += "address = '"+ h($(".address").eq(i).val()) +"'";
				sql += ",";
				sql += "tel = '"+ h($(".tel").eq(i).val()) +"'";
				sql += ",";
				sql += "fax = '"+ h($(".fax").eq(i).val()) +"'";
				sql += ",";
				sql += "account = '"+ h($(".account").eq(i).val()) +"'";
				sql += ",";
				sql += "meigi = '"+ h($(".meigi").eq(i).val()) +"'";
				sql += ",";
				sql += "is_bankshow = '"+ is_bankshow +"'";
				sql += ",";
				sql += "is_schshow = '"+ is_schshow +"'";
				sql += ",";
				sql += "is_seko_show = '"+ is_seko_show +"'";
				sql += ",";
				sql += "orderc = '"+ h($(".orderc").eq(i).val()) +"'";
				sql += ",";
				sql += "seko_invoice_no = '"+ h($(".seko_invoice_no").eq(i).val()) +"'";
				sql += ",";
				sql += "biko = '"+ h($(".biko").eq(i).val()) +"'";
...
//----------------------個別領域 ここから----------------------------
			...
			//チェックボックスハンドリング
			var is_bankshow = $(".is_bankshow").eq(0).attr("checked");
			if(is_bankshow == "checked")
				is_bankshow = 1;
			else
				is_bankshow = 0;

			var is_schshow = $(".is_schshow").eq(0).attr("checked");
			if(is_schshow == "checked")
				is_schshow = 1;
			else
				is_schshow = 0;

			var is_seko_show = $(".is_seko_show").eq(0).attr("checked");
			if(is_seko_show == "checked")
				is_seko_show = 1;
			else
				is_seko_show = 0;
			...
			if(id) { //更新
				...
				sql += ",";
				sql += "atena = '"+ h($(".atena").eq(0).val()) +"'";
				sql += ",";
				sql += "postal = '"+ h($(".postal").eq(0).val()) +"'";
				sql += ",";
				sql += "address = '"+ h($(".address").eq(0).val()) +"'";
				sql += ",";
				sql += "tel = '"+ h($(".tel").eq(0).val()) +"'";
				sql += ",";
				sql += "fax = '"+ h($(".fax").eq(0).val()) +"'";
				sql += ",";
				...
				sql += "is_seko_show = '"+ is_seko_show +"'";
				sql += ",";
				sql += "orderc = '"+ h($(".orderc").eq(0).val()) +"'";
				sql += ",";
				sql += "seko_invoice_no = '"+ h($(".seko_invoice_no").eq(0).val()) +"'";
				sql += ",";
				sql += "biko = '"+ h($(".biko").eq(0).val()) +"'";
				sql += " WHERE "+MAIN_ID_FIELD+" ="+id+";;";
				
			}
			else {
				//新規
				sql += "INSERT INTO "+MAIN_TABLE+" (seko, seko_nik,atena,postal,address,tel,fax, account, meigi, is_bankshow, is_schshow, is_seko_show, orderc, seko_invoice_no, biko) VALUES ( ";
				sql += "'"+h($(".seko").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".seko_nik").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".atena").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".postal").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".address").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".tel").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".fax").eq(0).val())+"'";
				...
				sql += "'"+is_seko_show+"'";
				sql += ",";
				sql += "'"+h($(".orderc").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".seko_invoice_no").eq(0).val())+"'";
				sql += ",";
				sql += "'"+h($(".biko").eq(0).val())+"'";
				sql += ");;";
			}
