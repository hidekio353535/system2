...
if($COMPANY_INVOICE_NO) {
	$my_invoice_no = "登録番号 ".$COMPANY_INVOICE_NO . "<br>";
}
...
while ($row = mysql_fetch_object($query)) {
	...
	$seko_invoice_no = "";
	if($row->seko_invoice_no) {
		$seko_invoice_no = "登録番号 ".$row->seko_invoice_no;
	}
...
/********************  発注書  **************************************************************/
	...
	if($num_s) {
		while ($row_s = mysql_fetch_object($query_s)) {
			...
			if($len > 40) {
				$m3[$cnt] = '<span class="size6">'.$row_s->g_genba."</span>";
			}
			else {
			...
			$m8[$cnt] = ceil($row_s->s_hattyu / (1 + get_tax($row->i_inv_date) / 100));
			$m9[$cnt] = $row_s->s_hattyu - $m8[$cnt];
	...
	if($h_chosei_name != "" && is_numeric($h_chosei) && $h_chosei != 0) {
		...
		$h_chosei_wo_stax = ceil($h_chosei / (1 + get_tax($row->h_hat_date) / 100));
		$total_wo_stax += $h_chosei_wo_stax;
		$h_chosei_stax = $h_chosei - $h_chosei_wo_stax;
		$total_stax += $h_chosei_stax;

		$total = "￥".number_format($total);
		$shokei = "￥".number_format($shokei);
		$total_stax = "￥".number_format($total_stax);
		$total_wo_stax = "￥".number_format($total_wo_stax);
		$h_chosei = "￥".number_format($h_chosei);
		$h_chosei_wo_stax = "￥".number_format($h_chosei_wo_stax);
		$h_chosei_stax = "￥".number_format($h_chosei_stax);
		$chosei_flag = true;
	}
	else {
		$total = $shokei;
	...
	if($chosei_flag) {
		...
		$h_chosei_wo_stax = preg_replace('/\-/','▲',(string)$h_chosei_wo_stax);
		$h_chosei_stax = preg_replace('/\-/','▲',(string)$h_chosei_stax);
	}
...
EOD;
		...
		<table width="640" border="0" cellpadding="4" class="size5">
			<tr>
				<td width="70" class="waku bgkuro tac">No.</td>
				<td width="25" class="waku bgkuro tac">リ新</td>
				<td width="30" class="waku bgkuro tac">工事日</td>
				<td width="140" class="waku bgkuro tac">現場名</td>
				<td width="160" class="waku bgkuro tac">現場住所</td>
				<td width="30" class="waku bgkuro tac">区分</td>
				<td width="60" class="waku bgkuro tac">金額(税抜)</td>
				<td width="50" class="waku bgkuro tac">消費税</td>
				<td width="60" class="waku bgkuro tac">金額(税込)</td>
				<td width="60" class="waku bgkuro tac">備考</td>
			</tr>
EOD;
...
EOD;
	...
	//最後のページに付加する
		...
		$tbl .= <<<EOD
			<tr>
				<td colspan="6" class="waku tac">{$h_chosei_name}</td>
				<td colspan="1" class="waku tar">{$h_chosei_wo_stax}</td>
				<td colspan="1" class="waku tar">{$h_chosei_stax}</td>
				<td colspan="1" class="waku tar">{$h_chosei}</td>
				<td colspan="1" class="waku"></td>
			</tr>
			<tr>
				<td colspan="6" class="waku2 tac">発注金額</td>
...
EOD;
		...
		$tbl .= <<<EOD
			<tr>
				<td colspan="10" class="tar">Page: {$page_total} / {$page_total}</td>
			</tr>
		</table>
EOD;
	$pdf->AddPage();
...
EOD;
		...
		<table width="640" border="0" cellpadding="4" class="size5">
			<tr>
				<td width="60" class="waku bgkuro tac">No.</td>
				<td width="40" class="waku bgkuro tac">開始日</td>
				<td width="140" class="waku bgkuro tac">現場名</td>
...
EOD;
	...
	//最後のページに付加する
		...
		$tbl .= <<<EOD
			<tr>
				<td colspan="5" class="waku tac">{$h_chosei_name}</td>
				<td colspan="1" class="waku tar">{$h_chosei_wo_stax}</td>
				<td colspan="1" class="waku tar">{$h_chosei_stax}</td>
				<td colspan="1" class="waku tar">{$h_chosei}</td>
				<td colspan="1" class="waku"></td>
			...
			<tr>
				<td colspan="5" class="waku2 tac">御請求金額</td>
				<td colspan="1" class="waku2 tar">{$total_wo_stax}</td>
				<td colspan="1" class="waku2 tar">{$total_stax}</td>
				<td colspan="1" class="waku2 tar">{$total}</td>
				<td colspan="1" class="waku"></td>
...
EOD;
		...
		$tbl .= <<<EOD
			<tr>
				<td colspan="9" class="waku tal">【お振込口座】{$account} {$meigi}</td>
			</tr>
EOD;
		}
        $cdate = date("00y0mdHi");
