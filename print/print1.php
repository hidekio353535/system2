...
while ($row = mysql_fetch_object($query)) {
	...
	if($s_shime_date == '0000-00-00' || !$s_shime_date)
		$s_shime_date = '';
	else
	...
	if($num_s) {
		while ($row_s = mysql_fetch_object($query_s)) {
			...
			//値引きとマイナスの処理
			if(	preg_match('/値引|端数/',$row_s->m_meisho)) {
				$m2[$cnt] = "";
				$m3[$cnt] = "";
				$m4[$cnt] = "";
			}
	...
	for($page = 1;$page <= $page_total;$page++) {
		...
		if($page == 1) {
			...
			<table width="640" border="0" cellpadding="1">
				...
				<tr>
					<td class="tac size22"><span class="size16">請求</span> 明細書</td>
				</tr>
				<tr>
					<td class="tac">&nbsp;</td>
				</tr>
				...
				<tr>
							...
							<tr>
								...
								&nbsp;<br />
									<table style="font-size:14pt;">
										<tr>
										<td width="160">御請求金額(税込)：</td>
										<td width="140" class="tac">{$top_inv}</td>
										</tr>
									</table>
								</td>
							</tr>
					...
					<td class="" width="230">
						...
						<table width="230" border="0" cellpadding="2">
							...
							<tr>
								<td class="size13" width="180">{$COMPANY_NAME}</td>
...
EOD;
		...
		$tbl .= <<<EOD
			<tr>
				...
				<td colspan="2" class="waku-t waku-b waku-r waku-l tal">
					<span style="font-size:10pt">振込先<br>
					埼玉りそな銀行<br>
					本川越支店<br>
					普通　4300466<br>
					ＧＲＡＮＺ株式会社</span>
