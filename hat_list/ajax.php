...
//----------------------個別領域 ここから----------------------------
$VIEW_TABLE = "matsushima_slip_hat";
$VIEW_ID_FIELD = "s_id";

$MAIN_TABLE = "matsushima_slip_hat";
$MAIN_ID_FIELD = "s_id";
...
//----------------------個別領域 ここから----------------------------
	...
	if($search_sel_seko != "" && $search_sel_seko != 0)
		$JYOKEN .= " AND (s_seko_id = '{$search_sel_seko}') ";

	$search_st_date = $parm[9];
	$search_end_date = $parm[10];
	$date_sel = $parm[11];
	
	if($date_sel == "work_date" && $search_st_date && $search_end_date) {
		$JYOKEN .= " AND (
						(
							(s_st_date is not null AND s_st_date != '0000-00-00' AND s_end_date is not null AND s_end_date != '0000-00-00')
							AND
							(
								(s_st_date <= '{$search_st_date}' AND s_end_date >= '{$search_end_date}') 
								OR 
								(s_st_date >= '{$search_st_date}' AND s_end_date <= '{$search_end_date}') 
								OR 
								(s_st_date <= '{$search_st_date}' AND s_end_date >= '{$search_st_date}' AND s_end_date <= '{$search_end_date}') 
								OR 
								(s_end_date >= '{$search_end_date}' AND s_st_date >= '{$search_st_date}' AND s_st_date <= '{$search_end_date}')
							)
						)
						OR
						(
							(s_st_date is not null AND s_st_date != '0000-00-00' AND (s_end_date is null OR s_end_date = '0000-00-00'))
							AND
							(s_st_date >= '{$search_st_date}' AND s_st_date <= '{$search_end_date}') 
						)
	...
	$search_sel_moto = $parm[13];
	if($search_sel_moto != "" && $search_sel_moto != 0)
		$JYOKEN .= " AND (g_moto_id = '{$search_sel_moto}') ";

	$search_sel_kubun = $parm[14];
	if($search_sel_kubun != "" && $search_sel_kubun != 0)
...
//----------------------個別領域 ここまで----------------------------
	...
	try {
		$sql = "SELECT *,
						(SELECT t.t_tantou FROM matsushima_tantou as t WHERE t.t_id = matsushima_genba.g_tantou_sub_id) as sub_tantou

		FROM {$VIEW_TABLE} 
		LEFT OUTER JOIN matsushima_genba ON matsushima_genba.g_id = {$VIEW_TABLE}.s_genba_id
		LEFT OUTER JOIN matsushima_kouji_syu_hat ON matsushima_kouji_syu_hat.sy_id = {$VIEW_TABLE}.s_seko_kubun_id
		...
		if($JYOKEN) {
			$_SESSION['csv_sql'] = 
				...
				LEFT OUTER JOIN matsushima_seko ON matsushima_seko.seko_id = {$VIEW_TABLE}.s_seko_id
				LEFT OUTER JOIN matsushima_kouji_syu_hat ON matsushima_kouji_syu_hat.sy_id = {$VIEW_TABLE}.s_seko_kubun_id
				LEFT OUTER JOIN matsushima_tantou ON matsushima_tantou.t_id = matsushima_genba.g_tantou_id
				LEFT OUTER JOIN matsushima_nai_1 ON matsushima_genba.g_nai1_id = matsushima_nai_1.nai1_id
...
//----------------------個別領域 ここから----------------------------
	...
	showTh("g_id", "現場ID");
	showTh("s_id", "発注ID");
	showTh("s_jv_rel_id", "JVID");
	showTh("nai1_nik", "リ新");
	showTh("sy_name_nik", "区分");
	showTh("g_genba", "現場名");
	showTh("g_genba_address", "現場住所");
	showTh("g_moto_id", "元請");
	showTh("s_seko_id", "発注先");
	showTh("g_tantou_id", "担当");
	showTh("sub_tantou", "福担当");
	showTh("s_date", "発注日", false); //非表示 display:none
...
//----------------------個別領域 ここから----------------------------
			...
			echo "<td class='nw tac'>";
				if( $row->s_jv_rel_id ) {
					show_clm($row->s_jv_rel_id, "s_jv_rel_id");
				}
			echo "</td>";
...
//----------------------個別領域 ここまで----------------------------
	...
	//区分
	$sql = "SELECT * FROM matsushima_kouji_syu_hat WHERE 1 ORDER BY sy_order";
	$query = @mysql_query($sql);
	$num = @mysql_num_rows($query);
	if(!$query) {
		echo "fail";
...
LEFT(kana,1) as kana3
	...
	if($slipno == 0)
		...
		$sql = "UPDATE matsushima_slip_hat SET s_is_print = 1 WHERE s_id in ({$id})";
		$query = mysql_query($sql);
	}
	else
		return;
	...
	if($slipno == 0)
		...
		$sql = "UPDATE matsushima_slip_hat SET s_is_print = 0 WHERE s_id in ({$id})";
		$query = mysql_query($sql);
	}
	else
		return;
