...
$PHEXCEL_PATH = "/PHPExcel-1.8/Classes";

require_once(dirname(__FILE__) .$PHEXCEL_PATH. '/PHPExcel.php');
require_once(dirname(__FILE__) .$PHEXCEL_PATH. '/PHPExcel/IOFactory.php');
...
//職方
$sheet->setCellValue("B3", $seko);

//日付
if($s_st_date && $s_st_date != "0000-00-00") {
	$year = date("Y", strtotime($s_st_date));
	$month = date("n", strtotime($s_st_date));
	$day = date("j", strtotime($s_st_date));

	$sheet->setCellValue("A4", $year . "年");
	$sheet->setCellValue("B4", $month);
	$sheet->setCellValue("D4", $day);
}

//施工区分
$sheet->setCellValue("H4", $sy_name);
//元請名
$sheet->setCellValue("B5", $moto);
//現場名
$sheet->setCellValue("B6", $g_genba);
//現場住所
$sheet->setCellValue("B7", $g_genba_address);
//主担当・副担当
$sheet->setCellValue("B8", $syu_tantou);
$sheet->setCellValue("F8", $sub_tantou);
//m2
$sheet->setCellValue("B9", $g_m2);
//ID
$sheet->setCellValue("H6", $id);

//出力ファイル名
$filename = "daily_report_{$id}.xlsx";
